...
static int do_import(const std::string& dev, int tape_file, int bf_hint, int mbf) {
    ...
    int dtf = 0;
    if (!tape.append(mbf, nullptr, [&](int) { return idx.serialize(-1, mbf); }, dtf)) {
...
static int do_append(const std::string& dev, const std::string& tarpath, int bf, int mbf) {
    ...
    const bool ok = tape.append(
        ...
        dtf);
    if (!ok) { std::fprintf(stderr, "mktapir: append failed\n"); return 1; }
...
int main(int argc, char** argv) {
    ...
    if (argc >= 3 && std::strcmp(argv[1], "import") == 0) {
        ...
        for (int i = 3; i < argc; ++i) {
            ...
            else if ((a == "-b" || a == "--block-factor")          && i + 1 < argc) bf  = std::atoi(argv[++i]);
            else if ((a == "-m" || a == "--manifest-block-factor") && i + 1 < argc) mbf = std::atoi(argv[++i]);
        }
        if (dev.rfind("/dev/", 0) != 0 || tape_file < 0) { usage(argv[0]); return 2; }
    ...
    if (argc >= 4 && std::strcmp(argv[1], "append") == 0) {
        ...
        for (int i = 4; i < argc; ++i) {
            const std::string a = argv[i];
            if      ((a == "-b" || a == "--block-factor")          && i + 1 < argc) bf  = std::atoi(argv[++i]);
            else if ((a == "-m" || a == "--manifest-block-factor") && i + 1 < argc) mbf = std::atoi(argv[++i]);
        }
        if (dev.rfind("/dev/", 0) != 0) { usage(argv[0]); return 2; }
