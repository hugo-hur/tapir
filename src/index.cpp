...
Node* Index::parent_of(const std::vector<std::string>& parts) {
    ...
    for (std::size_t i = 0; i + 1 < parts.size(); ++i) {
        ...
        if (it == cur->children.end() || !it->second->is_dir) return nullptr;
        cur = it->second.get();
    }
    return cur;
}
...
void Index::add_file(const std::string& path, uint64_t size, const std::string& sha256,
                     int dtf, int bf) {
    Node* n = ensure_path(path, false);
    if (!n) return;                       // already indexed — keep the existing entry
    n->size = size;
    n->sha256 = sha256;
    n->data_tape_file = dtf;
    n->block_factor = bf;
    if (meta_.find(dtf) == meta_.end()) {   // register this archive's header metadata
        Meta m;
...
void Index::load(const std::string& text) {
    ...
    for (const auto& arc : root) {
        ...
        meta_[dtf] = m;
        if (first) { source = m.source; created = m.created; first = false; }
...
static void collect(const Node* n, const std::string& prefix,
    ...
    for (const auto& [name, child] : n->children) {
        ...
        if (child->is_dir) collect(child.get(), p, out);
        else out.emplace_back(p, child.get());
    }
}
...
std::string Index::serialize(int new_dtf, int new_bf) const {
    ...
    std::tm tmv{};
    if (gmtime_r(&t, &tmv)) std::strftime(now, sizeof now, "%Y-%m-%dT%H:%M:%S", &tmv);
    ...
    for (const auto& [dtf, v] : groups) {
        ...
        uint64_t total = 0;
        for (const auto& pr : v) total += pr.second->size;
