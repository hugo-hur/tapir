...
bool tar_read_member(struct archive* a, const std::string& member, std::string& out) {
    ...
    while (archive_read_next_header(a, &e) == ARCHIVE_OK) {
        if (match(epath(e), member)) {
            out.clear();
            const void* b; size_t n; la_int64_t off; int r;
...
bool tar_write_files(struct archive* a, const std::vector<OutFile>& files) {
    ...
    for (const auto& f : files) {
        ...
        if (archive_write_header(a, e.get()) != ARCHIVE_OK) return false;
        off_t off = 0;
        ...
        while (remaining > 0) {
            ...
            if (archive_write_data(a, buf.data(), static_cast<size_t>(got)) < 0) return false;
            off += got;
...
bool tar_write_member(struct archive* a, const std::string& member, const std::string& data) {
    ...
    archive_entry_set_perm(e.get(), kFileMode);
    if (archive_write_header(a, e.get()) != ARCHIVE_OK) return false;
    if (!data.empty() && archive_write_data(a, data.data(), data.size()) < 0) return false;
    return true;
}

static std::string normalize(const char* p) {
    std::string s = p ? p : "";
    while (s.rfind("./", 0) == 0) s.erase(0, 2);   // strip leading "./"
    while (!s.empty() && s[0] == '/') s.erase(0, 1); // strip leading "/"
    return s;
}
...
bool tar_copy_members(
    ...
    while ((r = archive_read_next_header(in, &e)) == ARCHIVE_OK) {
        ...
        if (archive_write_header(out, e) != ARCHIVE_OK) return false;
        const bool is_file = archive_entry_filetype(e) == AE_IFREG && archive_entry_hardlink(e) == nullptr;
        Sha256 sha;
        uint64_t total = 0;
        const void* b; size_t n; la_int64_t off; int rr;
        while ((rr = archive_read_data_block(in, &b, &n, &off)) == ARCHIVE_OK) {
            if (archive_write_data(out, b, n) < 0) return false;
            if (is_file) { sha.update(b, n); total += n; }
        }
        if (rr != ARCHIVE_EOF) return false;
        if (is_file) cb(name, sha.hex(), total);
    }
    return r == ARCHIVE_EOF;
}
...
bool tar_for_each_member(
    ...
    while ((r = archive_read_next_header(a, &e)) == ARCHIVE_OK) {
        if (archive_entry_filetype(e) != AE_IFREG) { archive_read_data_skip(a); continue; }
        Sha256 sha;
        const void* b; size_t n; la_int64_t off; int rr; uint64_t total = 0;
