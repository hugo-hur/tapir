...
#include "writer.hpp" // WriterThread must be complete: the broker accessors touch its privates
#include "index.hpp"
#include "tape.hpp"
#include "tar_io.hpp"
...
namespace tapir
{
    ...
    // task subclass reaches the writer through them. WriterThread is complete here
    // (writer.hpp is included), so the accessors compile.
    ...
    std::unique_ptr<WriterTask> make_write_member_task(Node *node, std::shared_ptr<Staged> data,
                                                       std::string path, time_t mtime)
    {
        return std::make_unique<WriteMemberTask>(node, std::move(data), std::move(path), mtime);
    }

    std::unique_ptr<WriterTask> make_sync_task(std::future<void> &out_future)
    {
        auto task = std::make_unique<SyncTask>();
        out_future = task->get_future();
        return task;
    }
